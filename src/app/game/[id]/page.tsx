...
interface TeamData {
  name: string;
  players: { name: string; individualScore: number }[];
}
...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    socket.on('answer_result', (data: { correct: boolean; points: number }) => {
      setAnswerResult(data.correct ? `✅ +${data.points} очков` : '❌ 0 очков');
    });

    socket.on('round_result', (data: { teamScores: { teamA: number; teamB: number } }) => {
  ...
  return (
    <main className="min-h-screen bg-gray-900 p-4">
      ...
      <div className="bg-gray-800 rounded-2xl p-8 mb-6 text-center">
        <span className="text-[120px] font-bold text-white animate-bounce-in">
          {gameData.symbol}
        </span>
      </div>
